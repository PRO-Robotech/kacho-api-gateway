...
Test-first note (workspace CLAUDE.md §Запреты #12):
  These cases are written RED-first. They fail until:
    (a) kacho-iam PR #70 (InternalClusterService handler) is merged AND
    (b) THIS PR (kacho-api-gateway KAC-196) registers the routes on the
        internal mux AND adds the 4 catalog entries.
  Do NOT weaken assertions to make them pass — fix the implementation.
"""
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-GET-OK  (acceptance §6.00, KAC-196-00)
# ---------------------------------------------------------------------------
...
# D-11 gate: tokenAdmin carries `system_admin` → `admin` computed → PASS.
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-GET-403-ORDINARY  (acceptance §6.01)
# ---------------------------------------------------------------------------
...
# (D-11) fails OpenFGA Check `(user:U3, admin, cluster:cluster_kacho_root)`.
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-GRANT-OK  (acceptance §6.02, KAC-196-02)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-GRANT-403-ORDINARY  (acceptance §6.04)
# ---------------------------------------------------------------------------
# Setup: U3 (ordinary) tries to Grant himself admin → 403 PermissionDenied via D-11 gate.
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-GRANT-400-INVALID-USER  (acceptance §6.06, D-9)
# ---------------------------------------------------------------------------
# Setup: S grants a `usr_<...>` id that does not exist in kacho_iam.users.
# D-9: user existence check inside the handler TX → InvalidArgument with
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-GRANT-OK-IDEMPOTENT  (acceptance §6.03, D-4)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-REVOKE-OK  (acceptance §6.07)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-REVOKE-403-SELF  (acceptance §6.08, D-5)
# ---------------------------------------------------------------------------
# Setup: S (system_admin) revokes herself.
# D-5: self-revoke is rejected at the SQL CAS level → FailedPrecondition with
...
# acceptance §5.5 list naming, but the on-wire status code is 400 + grpc=9.
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-REVOKE-403-LAST  (acceptance §6.09, D-6)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-REVOKE-404-NOT-ADMIN  (acceptance §6.12, D-12)
# ---------------------------------------------------------------------------
# Setup: U3 has NEVER been a cluster-admin (no active row in cluster_admin_grants).
# D-12 (asymmetric with Grant idempotency): Revoke of a non-admin returns NotFound
# verbatim `"user usr_<...> is not an active cluster admin"`. (The exact text
# format from acceptance §6.12 — the {{userOrdinaryId}} placeholder is
# substituted at test time; assertion matches by code+regex below since the id
# value is dynamic.)
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-LIST-OK  (acceptance §6.11)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-LIST-403-ORDINARY  (acceptance §5.5, D-11)
# ---------------------------------------------------------------------------
# Setup: U3 (ordinary) tries to ListAdmins → 403 via D-11 gate.
...
# ---------------------------------------------------------------------------
# CLUSTER-ADMIN-INTERNAL-NOT-ON-EXTERNAL-TLS  (acceptance §5.5, workspace §Запрет 6)
# ---------------------------------------------------------------------------
