...
Test-first note (workspace CLAUDE.md §Запрет #12):
  RED-first. SEC-E-GATEWAY-MTLS-A / -NEG-A assert the public contract is preserved
  over the mTLS-profiled gateway. They fail if backend-dial mTLS breaks the JWT →
  principal → Check → backend chain (e.g. principal metadata dropped over mTLS, or
  Check no longer reached). Do NOT weaken assertions — fix the wiring.
"""
...
# ---------------------------------------------------------------------------
# SEC-E-GATEWAY-MTLS-A  (acceptance §3.4 / §3.13 happy)
# ---------------------------------------------------------------------------
...
#   mTLS→iam → director dials vpc over mTLS → 200 + Operation.
...
# ---------------------------------------------------------------------------
# SEC-E-GATEWAY-MTLS-NEG-A  (acceptance §3.13 negative)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SEC-E-GATEWAY-MTLS-BAN6-ADDRESSPOOL  (acceptance §3.8 / §3.13 ban #6 control)
# ---------------------------------------------------------------------------
...
# dial side does not open Internal.* on external (isolation is listener/director
