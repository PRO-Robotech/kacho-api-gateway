...
// middleware (KAC-127 Phase 3, Problem 1).
//
...
// permission catalog. The previous fallback (`grpcMethodForPath`) returned
// `"/" + path`, which never matches catalog keys — every authenticated REST
...
// the per-RPC latency budget (acceptance §4.4).
package middleware
