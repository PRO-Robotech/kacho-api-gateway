// subject_extractor.go — Subject extraction from verified Phase 2 JWT claims
// (KAC-127 Phase 3).
//
...
// token_hook in Phase 2) carry the kacho-native principal id.
//
// Resolution priority:
//
...
//  4. `ext_claims.kacho_workload_id` (Federation/Workload identity — Phase 5
//     prep; harmless to recognise it early so we don't trip over it later).
...
// Subject FGA-prefix vocabulary. Mirrors the OpenFGA Authorization Model v2
// types (acceptance §4 DSL): user / service_account / workload (Phase 5).
const (
...
const (
	...
	// SubjectKindWorkload — federated Workload identity (Phase 5 forward-ref).
	SubjectKindWorkload
...
func (e *SubjectExtractor) Extract(t *VerifiedToken) (ResolvedSubject, bool) {
	...
	// 4. ext_claims.kacho_workload_id (Phase 5 prep).
	if ext != nil {
