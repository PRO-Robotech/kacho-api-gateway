...
// TestExtractByPathTemplate_VerbSuffix — KAC-198 regression: grpc-gateway
// `:verb` suffix-action segments (e.g. `/vpc/v1/subnets/{subnet_id}:add-cidr-blocks`)
// used to break the segment-by-segment matcher because the final segment
// `{subnet_id}:add-cidr-blocks` did not match `{name}` placeholder shape,
// causing fall-through to wildcard `*` and AUTHZ_DENY "no path: unscoped
// resource" 403 on every such RPC (AddCidrBlocks, RemoveCidrBlocks, etc).
