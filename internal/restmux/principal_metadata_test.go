...
// principal_metadata_test.go — sub-phase 5.4: the internal-mux re-dial must
// forward the validated JWT acr (X-Kacho-Token-Acr, set by the public DPoP
// middleware) as x-kacho-token-acr metadata, alongside the existing
// x-kacho-principal-* headers, so the iam internal acr-floor can enforce
// required_acr_min on the :9091 path.
...
// 5.4 gateway: principal metadata carries x-kacho-token-acr from the
// Grpc-Metadata-prefixed header the DPoP middleware sets.
...
// 5.4 gateway: the non-prefixed header variant is also accepted (robust).
...
// 5.4 gateway: absent acr ⇒ the key is simply not appended (no empty value);
// the iam floor then treats it as acr=0 (fail-closed) — the gateway forwards
// only what the validated token carried.
