...
// Internal-vs-external isolation: Internal* methods must never be reachable on
...
// selects the JSON marshaller; it never rejects by listener. Without this
// marker Internal* REST routes (/iam/v1/internal/*, /vpc/v1/addressPools,
// /vpc/v1/networks/{id}/internal, ...) would be reachable on the EXTERNAL
// endpoint: the gRPC routing (server.go Resolver / shimproxy.go) blocks
// Internal* via HasInternalSuffix, but the REST path needs an equivalent gate.
//
// This package mirrors that gRPC routing for REST: the external TLS
