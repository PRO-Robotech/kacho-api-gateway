// mtls_dial_bufconn_test.go — SEC-E in-memory mTLS handshake tests for the
// gateway backend-dial creds. RED-first: these exercise the creds produced by
// buildBackendDialCreds / cfg.EdgeTLSClient against a real corelib grpcsrv TLS
// server over a bufconn pipe — no docker, no network.
//
// Coverage:
//   - SEC-E-05 (§3.5): principal `x-kacho-principal-*` metadata propagates over
//     mTLS AND the server observes the client peer-cert SAN (both layers present,
//     orthogonal).
//   - SEC-E-09/§3.12: mTLS client dialing an insecure (plaintext) server →
//     fail-closed (Unavailable), never a silent plaintext fallback.
//   - SEC-E-11 (§3.11): server-cert signed by a foreign CA (not the gateway's
//     internal-CA bundle) → handshake reject → Unavailable.
package main
...
// SEC-E-05 — principal metadata + peer-cert SAN both observed over mTLS.
func TestSECE05_PrincipalAndPeerCertOverMTLS(t *testing.T) {
	...
	// Principal metadata set on the outgoing ctx, exactly as the director does.
...
// SEC-E-09 / §3.12 — mTLS client vs insecure server → fail-closed (Unavailable).
...
// SEC-E-11 — server presents a cert from a FOREIGN CA → handshake reject →
// Unavailable. The gateway never accepts an untrusted backend.
...
// SEC-E-07 — opsLoopback is always insecure regardless of enabled edges. The
// loopback creds come from a dedicated insecure dial-option, not the per-edge
// map. Asserts the helper that main.go uses returns insecure even when every
// edge is mTLS-on.
