...
// mtls_config.go — SEC-E per-edge backend-dial credential selection (acceptance
// §1.2, §3.1/§3.2/§3.3/§3.7/§3.9).
//
// The gateway opens one long-lived ClientConn per backend-domain key (vpc /
// vpcInternal / compute / … — see config.BackendAddrs). SEC-E maps each key to
// its mTLS *edge* (vpc | compute | iam | nlb), then builds per-edge transport
// credentials: the shared "api-gateway" client-cert (TLS) when the edge is
// enabled, or insecure when it is not (dev backward-compat).
//
// Contract:
//   - every edge disabled (default) ⇒ every dial insecure, identical to current
//     dev (SEC-E-01).
//   - an edge enabled with full cert material ⇒ TLS creds for every backend key
//     on that edge; other edges stay insecure (SEC-E-02/§3.9).
//   - an edge enabled with missing/partial cert material ⇒ error (fail-fast,
//     SEC-E-03); main.go log.Fatalf's so the process does not start with a
//     silent insecure fallback (epic §6.7).
//   - the "operation" self-loopback is NOT a backend edge — it is dialed
//     in-process and is always insecure (SEC-E-07, §2.3).
...
// backendKeepalive — KAC-244: keep idle inter-service conns warm (kind kills idle
// conns faster than a 30s interval). Shared by every backend dial.
...
// dialBackends is the SEC-E composition-root helper that opens one lazy
// ClientConn per backend-domain key with that key's per-edge transport creds
// (mTLS when its MTLS_<EDGE>_ENABLE flag is set + cert material present, else
// insecure — dev backward-compat), plus the always-insecure "operation"
// self-loopback ClientConn (in-process, never crosses a pod boundary — SEC-E-07).
//
// It is fail-fast: an enabled edge missing cert material aborts the whole build
// (the process must not come up half-secured — SEC-E-03, epic §6.7); main.go
// log.Fatalf's on the returned error. Each ClientConn carries the shared
// keepalive + round-robin service-config (unchanged from the pre-SEC-E wiring).
//
...
func dialBackends(cfg config.Config) (proxy.Backends, func(), error) {
	...
	// operation self-loopback — always insecure (in-process re-entry, SEC-E-07).
...
// not apply (SEC-E-07, §2.3) regardless of which edges are enabled.
...
// edge and share MTLS_IAM_ENABLE (OQ-SEC-E-3). Fail-fast on misconfig.
