...
## newman-e2e.yml — full-stack authz E2E (on-demand)

Поднимает весь umbrella-стек (Postgres + Ory + OpenFGA + все доменные сервисы) на
локальном kind-кластере и гоняет authz-матрицы через REST api-gateway. Зависит от
наличия соседних репозиториев и тяжёлый (~15-30 мин), поэтому запускается **вручную**
(`workflow_dispatch`) или с центрального стенда — push-гейты валидируют сам репо
быстрыми workflow выше.
